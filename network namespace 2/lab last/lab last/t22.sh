...
# Change only the bandwidth (both ways) of the link between the Receiver and Router
...
sudo ip netns exec router tc qdisc add dev eth2 root netem rate 2mbit burst 32kbit latency 400ms
sudo ip netns exec Receiver tc qdisc add dev eth3 root netem rate 2mbit burst 32kbit latency 400ms
