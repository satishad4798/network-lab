...
//	std::cout << "TCP TcpLedbat from H1 to H4" << std::endl;
	...
	Ptr<Socket> ns3TcpSocket1 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(0), port), port, "TcpLedbat", senders.Get(0), receivers.Get(0), oneFlowStart,500, packetSize, numPackets*10, transferSpeed, oneFlowStart, 500);
...
//	std::cout << "TCP Westwood from H2 to H5" << std::endl;
	...
	Ptr<Socket> ns3TcpSocket2 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(1), port), port, "TcpNewReno", senders.Get(1), receivers.Get(1), 25, 100, packetSize, numPackets, transferSpeed,25, 100);
	...
	Ptr<Socket> ns3TcpSocket3 = uniFlow(InetSocketAddress(receiverIFCs.GetAddress(2), port), port, "TcpNewReno", senders.Get(2), receivers.Get(2), 25, 100, packetSize, numPackets, transferSpeed,25,100);
